...
func main() {
	...
	r.Post("/auth/verify/resend", func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("Токен подтверждения для %s: %s (перейдите по ссылке: %s/verify?token=%s)", user.Email, verificationToken, verifyURL, verificationToken)
		w.Header().Set("Content-Type", "application/json")
	...
	r.Post("/series", func(w http.ResponseWriter, r *http.Request) {
		...
		err = pool.QueryRow(r.Context(),
			`INSERT INTO series (title, description, category_id, cover_url, uploaded_by) VALUES ($1, $2, $3, $4, $5) RETURNING id`,
			title, descPtr, categoryID, coverURL, userID).Scan(&seriesID)
		if err != nil {
			log.Printf("CreateSeries: %v", err)
		...
		json.NewEncoder(w).Encode(map[string]int64{"id": seriesID})
	})
	...
	r.Put("/series/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = pool.Exec(r.Context(),
			`UPDATE series SET title = $2, description = $3, category_id = $4, cover_url = $5 WHERE id = $1`,
			id, title, descPtr, categoryID, coverURL)
		if err != nil {
	...
	r.Delete("/episodes/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = pool.Exec(r.Context(), `DELETE FROM episodes WHERE id = $1`, id)
		if err != nil {
